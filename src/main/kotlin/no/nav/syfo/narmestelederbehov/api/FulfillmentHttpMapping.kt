...
private val logger = applicationLogger(FulfillNarmestelederbehovResult::class.java)
private val invalidManagerContact = Event<ManagerContactValidationIssue>(
    name = "narmestelederbehov_invalid_manager_contact",
    level = Level.WARN,
    message = "Manager contact validation failed",
    operation = "fulfill_narmestelederbehov",
    fields = mapOf(
        "field" to { it.field.name },
...
fun FulfillNarmestelederbehovResult.requireFulfilled() {
    when (this) {
        ...
        is FulfillNarmestelederbehovResult.InvalidManagerContactDetails -> {
            issues.forEach { logger.event(invalidManagerContact, it) }
