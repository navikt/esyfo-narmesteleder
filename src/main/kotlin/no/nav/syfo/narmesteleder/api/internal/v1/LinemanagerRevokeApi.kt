...
/**
 * The auth plugin is installed on the DELETE method node. Ktor reuses routing nodes with equal
 * selectors, so installing it on `{id}` would also apply it to the sibling GET route that resolves
 * the same URI.
 *
